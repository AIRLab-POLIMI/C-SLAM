...
bool AnchoredRectangleHandler::addFeatureObservation(long int id, double t,
  ...
  if (!d.isInitialized) {
    ...
    if (d.initStrategy->initialize()) {
      ...
      auto it = d.zHistory.begin();

      // special case for the first observation
      const ObjectObservationDescriptor &obs = it->second;
      MeasurementEdgeWrapper_Ptr ret = _filter->addMeasurement(sensor+"_first", obs.t,
          obs.z, cov, obs.pose);
      assert(ret);
      //*/
      ...
      cerr << "[AnchoredRectangleHandler] Ready to estimate depth for track "
          << id << endl;
    }
  } else {
...
bool AnchoredRectangleHandler::getFeatureDimensions(long int id,
  ...
  ParameterWrapper_Ptr fohp_par = _filter->getParameterByName(sensor + "_FOhp"); // we need also inverse depth for computing actual dimensions
